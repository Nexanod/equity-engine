...
export function AddDecisionForm({ members }: { members: Member[] }) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [contributions, setContributions] = useState<{ memberId: string; influenceLevel: number }[]>(
    members.length > 0 ? [{ memberId: members[0].id, influenceLevel: 3 }] : []
  );
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) {
      ...
      setError(data.error ?? "Failed to add decision");
      return;
    }
    setOpen(false);
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-lg rounded-xl border border-zinc-800 bg-zinc-900 p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-white">Add strategic decision</h2>
            <form onSubmit={submit} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <input name="importanceWeight" type="number" min={1} max={10} defaultValue={5} className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-white" />
              </div>
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label className="text-sm text-zinc-400">Contributors (influence level 1–5)</label>
